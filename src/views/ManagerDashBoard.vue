    ...
    <v-dialog v-model="shiftDialog.open" max-width="640" eager>
      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <v-alert
            v-if="selectedUnavailableWorkers.length"
            type="warning"
            variant="tonal"
            density="comfortable"
            class="mt-3"
          >
            Selected but unavailable: {{ selectedUnavailableWorkers.join(", ") }}
          </v-alert>
...
export default {
  ...
  computed: {
    ...
    selectedUnavailableWorkers() {
      const unavailableByID = new Set(this.unavailableWorkerItems.map((item) => Number(item.value)));

      return (this.shiftDialog?.form?.assignedWorkerIDs || [])
        .map((value) => Number(value))
        .filter((id) => unavailableByID.has(id))
        .map((id) => this.workersByID[id]?.name || `Worker ${id}`);
    },
  ...
  methods: {
    ...
    async loadUnavailability() {
      ...
      const workerIDs = new Set(Object.keys(this.workersByID).map((key) => Number(key)).filter((id) => Number.isFinite(id) && id > 0));
      ...
      this.unavailabilityBlocks = rows.map((row) => this.normalizeUnavailabilityBlock(row)).filter((block) => block.id && workerIDs.has(Number(block.userID)));
    ...
    async openEditShiftModal(event) {
      ...
      await Promise.all([this.loadAssignmentsForShift(shiftID), this.loadUnavailability()]);
